import type { TaskProjection } from "./projected-finish";

// Turns the same per-task projection data behind the live "projected
// finish" line (lib/projected-finish.ts) into a proportional horizontal
// timeline — one segment per task, width = share of the list's *current*
// total, not a fixed original total. See "Live task list timeline" in
...
export interface Timeline {
  segments: TimelineSegment[]; // in task order; zero-width (missed/rest) tasks are omitted
  totalMinutes: number;
  // Reconstructed real start instant (ms since epoch) — "now" minus every
  // done task's real actualMinutes and the active task's real elapsed so
  // far. Deliberately excludes pending tasks' projected time (they haven't
  // happened) and any paused/idle time (same simplification Story 3's
  // TaskListSession.totalActualMinutes already makes — see task-lists-api.md).
  startInstant: number;
}

export function computeTimeline(
  items: Array<{ id: string } & TaskProjection>,
  ...
  const raw = items.map((item) => {
    ...
    if (item.state === "active") {
      ...
      // one source of truth for "how far into this task are we really."
      const elapsed =
