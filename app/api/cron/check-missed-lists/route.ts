...
// docs/features/notifications.md. Unauthenticated (no user session): this
// route's only auth boundary is the QStash request signature, same
// conceptual role assertNfcVerified plays for a scan. Runs every company in
// one invocation — fine at current scale, revisit only if company count
// grows enough that this risks the function's max duration (see the doc's
// "Batching" note).
