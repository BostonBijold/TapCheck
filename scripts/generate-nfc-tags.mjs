...
// can be pre-programmed (with Boston's own NFC writer app) with a working
// URL before any user owns it. See docs/features/nfc.md.
//
...
// server.url; update both together if the domain ever changes (see the
// "Known follow-up" note in app/.well-known/apple-app-site-association).
...
const DOMAIN = "be-one-nu.vercel.app";
