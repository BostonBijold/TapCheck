...
// Shared field-list editor for a form_check item's formFields — used by both
// AddHabitSheet (creating a new check) and RoutineEditView (editing one).
export default function CheckFieldsEditor({ fields, onChange }: Props) {
    ...
    <div>
      ...
      {fields.length === 0 && (
        <p className="font-mono text-[10px] text-dim mt-2">
          Add at least one field — a number reading or a yes/no check.
        </p>
      )}
    </div>
  );
