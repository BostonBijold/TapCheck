...
export default function TaskRow({
  ...
  const [backMins, setBackMins] = useState(
    item.taskType === "stopwatch" ? "30" : String(item.projectedMinutes)
  );
  const [editingTime, setEditingTime] = useState(false);
  const [startTime, setStartTime] = useState("");
  const [endTime, setEndTime] = useState("");
  // Back-entry field capture for a form task — a retroactive log still
  // needs its readings, not just a duration (see TaskFormScreen.tsx for the
  ...
  // Compact inline field inputs for logging a form task back-dated —
  // same field shape TaskFormScreen renders full-screen for a live session,
  // just condensed to fit inside this row's expanded action panel.
  const backEntryFields = isForm && (
