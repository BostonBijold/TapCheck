...
| `models/` | Mongoose schema/model definitions, one per MongoDB collection (`Goal.ts`, `HabitTemplate.ts`, `Philosophy.ts`, `RoutineGroup.ts`, `RoutineItem.ts`, `RoutineLog.ts`, `RoutineSession.ts`, `Todo.ts`, `User.ts`, `Virtue.ts`, `VirtueCheckIn.ts`, `AppIntentLink.ts` — see app-intents.md). |
...
| `app/api/` | Every backend endpoint, grouped by domain (`routines`, `routine-items`, `routine-logs`, `routine-review`, `habits`, `habit-templates`, `goals`, `todos`, `philosophies`, `virtues`, `virtue-checkins`, `analytics`, `user`, `external`, `auth`, `seed`, `dev`). `external` is API-key-authenticated (no session), for outside callers like the native App Intents "Trigger Habit" action — see `docs/api/external-api.md`. |
