...
| `components/` | Shared React client components — page-level views (`RoutinesView.tsx`, `GoalsView.tsx`, …), bottom-sheet/modal flows (`AddHabitSheet.tsx`, `FABTaskSheet.tsx`, …), smaller presentational pieces (`HabitIcon.tsx`, `StreakDots.tsx`, …), and the NFC feature's `DoneScreen.tsx`/`ClaimTagPicker.tsx`/`UniversalLinkHandler.tsx` (see nfc.md). |
...
| `ios/` | Generated Xcode project for the native shell (Capacitor). `ios/App/App.xcodeproj` is the project to open — this Capacitor version uses Swift Package Manager, not CocoaPods, so there is no `.xcworkspace`. `ios/App/App/MainViewController.swift` is the one piece of hand-written native code (disables WebView scroll bounce); `ios/App/App/App.entitlements` declares `com.apple.developer.associated-domains` for the NFC feature's Universal Links (see nfc.md) — everything else is Capacitor-generated boilerplate. `ios/DerivedData/` is Xcode build cache, gitignored. |
...
**Deep linking:** `components/UniversalLinkHandler.tsx` (mounted in `app/layout.tsx`, `Capacitor.isNativePlatform()`-guarded) listens for `@capacitor/app`'s `appUrlOpen` event and navigates the WebView to the tapped path — without it, Capacitor's native Universal Link handling only broadcasts a notification and never touches the WebView on its own. Currently only exercised by the NFC feature's `/nfc/[tagCode]` links; see nfc.md for the full story, including why this wasn't obvious from a clean `xcodebuild` exit.
...
| `app/api/` | Every backend endpoint, grouped by domain (`routines`, `routine-items`, `routine-logs`, `routine-review`, `habits`, `habit-templates`, `goals`, `todos`, `philosophies`, `virtues`, `virtue-checkins`, `analytics`, `user`, `external`, `auth`, `seed`, `dev`, `nfc-tags`). `external` is API-key-authenticated (no session), for outside callers like an iPhone Shortcut — see `docs/api/external-api.md`. |
