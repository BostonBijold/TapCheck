...
- **`end`** — called from `RoutinesView.tsx`'s `handleTimerComplete`/`handleTimerMissed` (standalone timer), and from three places in `RoutineSession.tsx`: the `advance()` summary branch, the foreground-revalidation effect's summary branch, and `handleClose` (the X button, which always flushes-and-exits, unlike closing the standalone `TimerScreen` — see below).

**Deliberately *not* called from `TimerScreen`'s plain `onClose`** (`setTimerItem(null)` in `RoutinesView.tsx`) — closing that screen leaves the log `in_progress` on the server, resumable later, and the whole point of a Live Activity is staying visible on the Lock Screen after the app itself is closed. Ending it there would defeat the feature for its most common case.
