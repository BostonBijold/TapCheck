...
**`content-state`'s `startedAt` is a Unix timestamp (a JSON number), not an ISO string** — Swift's default `Codable` synthesis for `Date` (`.deferredToDate`), which `RoutineActivityAttributes.ContentState` doesn't override, and which APNs delivers verbatim for ActivityKit to decode on-device. This is the one place in this feature where the wire format for the *same* struct differs depending on the transport: the local plugin's `parseContentState` reads an ISO string (matching JS's `Date#toISOString()`) because that request is JSON-encoded by hand in `LiveActivityPlugin.swift`'s own Capacitor call layer, not by `Codable`.
