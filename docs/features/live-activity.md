...
**Checkbox and `form_check` items** (see [`timer.md`](timer.md)) have no ring timer of their own; `RoutineSession.tsx`'s per-item effect calls `end` rather than `update` when landing on either (`isCheckboxItem || isFormCheckItem`), so the Lock Screen doesn't show a frozen, meaningless timer while the user is filling in a check's fields. Landing on the *next* timed item afterward goes through `update()`'s start-fallback, same as a session's very first item.
