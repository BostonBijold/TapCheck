...
# Routines (shift checks)

The Today/Routines page groups a restaurant's shift checks into `RoutineGroup`s (Opening Shift, Mid-Shift, Closing Shift, and any user-created "custom" groups), plus the standalone "Facility Checks" group for anytime/recurring checks — see [habits.md](habits.md). Each group contains an ordered list of `RoutineItem`s, and each item's status for a given date is tracked by a separate `RoutineLog` document, so history is never overwritten — every day gets its own log per item.

## Item types

`RoutineItem.itemType` is one of:

- **`form_check`** — the only creatable type. Has a `formFields` array (number readings with an optional unit/min/max, or yes/no items); tapping it opens `components/FormCheckScreen.tsx` to fill in those fields, with elapsed time tracked the same way a timer would be.
...
The Sunday-only "Routine Review" item type has been retired along with the feature it backed (goal-vs-actual-minutes tuning) — it doesn't fit a checklist-based work app.
...
- **pending** → tap opens the row and shows: Start Check (opens `FormCheckScreen`), or Missed/Rest buttons. If the group's scheduled window has passed ("back-entry" mode, see below), the check button is replaced by a Done button plus a manual minutes input (and, for a `form_check` item, the same field inputs).
...
- **missed** / **rest** → shows a retry action (Start Check, or Done+minutes if in back-entry mode) plus the other skip state and Undo.
...
Each shift group (except Facility Checks, see [habits.md](habits.md)) has a `startTime` (`HH:MM`) and an implied end time, `deriveCollapseAfter`, computed as `startTime + sum(projectedMinutes of all non-checkbox items)` (`components/RoutineGroupCard.tsx`). On today's view:
...
The shared math lives in `lib/routine-progress.ts`'s `computeWeeklyProgress` (imported by both `StreakDots` and the Analytics Check Breakdown, see `analytics.md`, so the two never diverge). Each of the week's 7 days classifies into one of six states — the first two are solid fills ("something happened"), the rest are hollow, no-fill outlines distinguished by border color/style ("needs a look"), deliberately so a close-but-different fill color is never the only thing telling two states apart:
...
Only applies to items with a real time target — a `form_check` item's fields aren't timed against a target the same way, so this tiering is mostly a legacy concern from the pre-pivot timer types.

The resulting percentage (`successCount / successThreshold * 100`) is **uncapped** — hitting the threshold with days to spare stays a win past 100%, it doesn't clamp back down. A three-state, non-gradient **pacing** verdict — `green` (threshold already reached), `red` (mathematically out of reach even with a perfect rest of the week: `successCount + remainingScheduled < successThreshold`), `amber` (still achievable, everything else) — drives the Analytics Check Breakdown's bar/badge (`StreakDots` itself doesn't surface pacing, only the per-day dots).

## Editing groups and items

- `components/RoutineEditView.tsx` (`app/(app)/routines/[groupId]/edit/page.tsx`) — dedicated per-group edit page: a drag-to-reorder item list, each item's inline edit form (name, icon, fields, schedule/threshold — see `CheckFieldsEditor.tsx`), the "Add check" sheet, and the group's `startTime`. Also displays each group's and item's raw Mongo `_id` read-only (`select-all`, no copy button) — these are the ids the [external API](../api/external-api.md) needs to target a specific timer. Its per-item inline edit form (`SortableRow`) is also where `scheduledDays`/`successThreshold` (see "Weekly schedule + success threshold" above) get edited after creation — a day-of-week toggle row plus a threshold input, clamped client-side to never exceed the number of selected days.
...
- **No UI creates, renames, reorders, or deletes a *group*** (only its `startTime`, via the schedule field above). `PATCH /api/routines/[groupId]` does accept a `name` field server-side, but nothing in the app currently sends one — groups are entirely seed-created (`lib/seed.ts`) today. Flagging as a real current limitation, not an oversight in this doc.

## The "Start Checks" sequential session

Tapping "Start Checks"/"Continue Checks" on a group (not shown for the Facility Checks group) opens `components/RoutineSession.tsx`, which steps through that group's items one at a time in a single full-screen flow rather than expanding rows individually. Each item gets its own server-side `in_progress` record as it becomes current, and closing the session mid-item flushes that item's progress rather than discarding it — full mechanics in [timer.md](timer.md).
...
- `components/RoutineGroupCard.tsx` — per-group card: collapse logic, completion check, renders `RoutineItemRow` (or `HabitItemCard` for the Facility Checks group).
...
- `components/RoutineEditView.tsx` — group/item management (also the path for editing a Facility Checks item — see [habits.md](habits.md)).
- `lib/routine-visibility.ts` — a visibility seam left over from the retired Sunday-only Routine Review item type; every current item type is always visible. `scheduledDays`/`successThreshold` (above) are a separate, unrelated concept — they never affect this function or Today-view visibility at all.
- `lib/routine-progress.ts` — the shared weekly-progress math (see "Weekly schedule + success threshold" above).
- `lib/seed.ts` — idempotent seeding of default shift groups/items for new users.
