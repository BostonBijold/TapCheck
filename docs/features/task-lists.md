...
**A FAB scan is a fourth way to start or join a shift-window list's session**, alongside tapping "Start Tasks"/"Continue Tasks" or resuming a paused task — see [nfc.md](nfc.md)'s "FAB 'scan to open' shortcut". Unlike those three, it never opens `TaskListSessionView`'s guided walkthrough, and it is permanently tied to exactly one task: it anchors just the one physically-scanned task to the list's session (so the session exists and "Continue Tasks" is available afterward) and opens that task standalone, only if that task is still untouched today — rescanning an already-logged task (any state — `in_progress`, `done`, `missed`, `rest`) never reopens or advances it, only reports its status. It's subject to the same lock as the other three: `lib/task-list-session-actions.ts`'s `resolveFabScanTarget` checks `getOpenSessionLocks` before answering, so a scan that lands on a list someone else already holds surfaces the same "In progress by `<name>`" state instead of silently joining.
