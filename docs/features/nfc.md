...
- **`{ mode: "anytime", taskId }`** — the task's list has no `startTime`, so nothing about session state applies. `BottomNav.tsx` navigates to `/tasks?openTaskId=<id>&verifiedNfcUid=<uid>&date=<localDate>`, and `TasksView.tsx`'s "Handle URL params passed from FAB navigation" effect picks up `autoOpenTaskId` there and opens that task exactly like tapping "Start task" on it directly — `setTimerItem`, no separate code path. This is the original, unchanged behavior.
- **`{ mode: "session", taskId, taskListId }`** — the task belongs to a shift-window list, and either no session is open for it today or the scanning user already holds the open one. `BottomNav.tsx` navigates to `/tasks?openSessionTaskId=<id>&openSessionListId=<listId>&verifiedNfcUid=<uid>&date=<localDate>` instead. The same effect's `autoOpenSessionTaskId`/`autoOpenSessionListId` branch finds that task's list and calls `setActiveSession({ taskList, startIndex })` — the identical call a manual "Continue Tasks" tap makes — routing straight into `TaskListSessionView` at the scanned task. No explicit session-creation call is needed anywhere: `ensureOpenSession` fires automatically the moment that view's normal flow writes the first `in_progress` log for the list/date, attributing the session to whoever's authenticated call made it — the scanning user, with zero special-casing, exactly as if they'd tapped "Continue Tasks" themselves.
- **`{ mode: "locked", taskId, taskListId, lockedByName }`** — the task's list already has an open session held by someone else. `resolveFabScanTarget` checks this via the existing `getOpenSessionLocks` before answering, since the one-person-at-a-time session lock is otherwise enforced only in the UI (`TaskListCard`'s button), not at the database-write level — a scan needed its own explicit check to avoid silently barging into someone else's active session. `BottomNav.tsx` shows `"In progress by <name> — try again once they finish."` in the same transient pill slot used for "no task linked"/scan-failed messages, with **no navigation at all** — a scan never fights an active lock.
- **`{ mode: "complete" }`** — the task's own list, and every other shift-window list, is already fully resolved for today. `BottomNav.tsx` shows "All shift lists are already complete for today." with no navigation.

**Redirecting to the nearest incomplete list**: if the scanned task's own list is already fully resolved for today (`isTaskListFullyResolved` — every active task in it has a terminal `done`/`missed`/`rest` log), `resolveFabScanTarget` doesn't just reopen that finished list — it looks across every other active shift-window list in the company via `resolveNearestIncompleteShiftList`, skips any that are *also* fully resolved, and picks whichever remaining one has a `startTime` closest to the client's `nowMinutes` (tie-break: list `order`) — the exact same "closest start time, tie-break by order" scoring `resolveMostRelevantPlacement` already uses for a single tag's multiple placements, just applied across the company's lists instead. The anchor task within that winning list is its own first not-yet-terminal task (`firstUnresolvedTaskInList`), the same target a manual "Continue Tasks" tap would land on. If every shift-window list is fully resolved, the result is `{ mode: "complete" }` instead. The scanned tag's own `verifiedNfcUid` is still forwarded to the client even when redirected this way — harmless, since `TaskFormScreen`'s pre-verification only ever short-circuits when the UID exactly matches *that* task's own `nfcTagUid`, so it silently falls through to a normal Save/Scan NFC step on a task the tag isn't actually bound to.
...
**This scan pre-satisfies that task's own Scan NFC step — two equivalent ways to finish a bound task, one scan either way**, in both the standalone (`"anytime"`) and session (`"session"`) paths:
- *Scan on the way in:* FAB → scan → task opens (standalone, or as the first task of an auto-joined/started session) → fill in fields → **Save** completes it immediately, no second scan.
- *Scan on the way out:* open the task normally (tap it in the list, or reach it later in a session) → fill in fields → **Scan NFC** scans at the end, as before.

Mechanically, for the standalone path: `autoOpenTaskId`'s URL also carries `verifiedNfcUid`, which `TasksView.tsx` stores as `preVerified: { taskId, uid }` — keyed by taskId, not just a bare uid, so it can never leak onto a *different* task opened some other way (tapping a task directly, resuming, session navigation all leave it `null`). `TaskFormScreen.tsx` receives it as `preVerifiedNfcUid`; when it matches `item.nfcTagUid`, `alreadyVerified` is true, the button just reads **Save**, its subtitle reads "Tag verified — Save to complete", and `handleSave` calls `onComplete(values, actualMinutes, preVerifiedNfcUid)` directly — no new `scanNfcTag()` call, but the server's `assertNfcVerified` check still runs and still requires that exact UID, same as the real-scan path. `preVerified` is cleared on completing, missing, or closing that task's screen, so it's single-use per open — reopening a task later (any path) always requires proving the tag again.

For the session path, the identical short-circuit is threaded through a separate prop instead, since it feeds a different child: `TasksView.tsx` stores `sessionPreVerified: { taskId, uid }` (kept apart from `preVerified` so it can never cross paths with the standalone screen) and passes it into `TaskListSessionView` as `preVerified`. That component seeds its own `armedVerification` state from it once, and disarms it (`setArmedVerification(null)`) the instant `currentIndex` moves off `startIndex` for any reason — advancing, jumping, or an external completion — so it only ever applies to the one task the scan actually read, and only on the session's very first task. Reopening that task later, even by jumping back to it in the same session, never reapplies it — same single-use rule as the standalone path.
