...
- **`{ mode: "session", taskId, taskListId }`** — the task belongs to a shift-window list, and either no session is open for it today or the scanning user already holds the open one. `BottomNav.tsx` navigates to `/tasks?openSessionTaskId=<id>&openSessionListId=<listId>&verifiedNfcUid=<uid>&date=<localDate>` instead. **This does not open `TaskListSessionView`'s guided walkthrough.** Each physical tag identifies exactly one task, never more than one, and the whole point of requiring a scan is proving the person is physically at *that* item right now — letting completion auto-advance into a different, unscanned task would defeat that. Instead, `TasksView.tsx`'s effect `POST`s `{ taskId, date, state: "in_progress", sessionTaskListId: taskListId }` directly (the same shape `TaskListSessionView`'s own per-task effect already sends) and opens the standalone `TaskFormScreen`/`TimerScreen` for that one task, exactly like the `"anytime"` path above. Setting `sessionTaskListId` is what makes `ensureOpenSession` start or join that list's session server-side (see `startInProgressLog`, `lib/task-log-actions.ts`) — no separate session-creation call needed — so `TaskListCard`'s "Start Tasks" button correctly reads "Continue Tasks" afterward, even though only this one task was ever touched. Completing (or missing) the task closes back to the Today view, same as any standalone task — the rest of the list stays exactly as it was, waiting for its own tag or a manual "Continue Tasks" tap.
...
**This scan pre-satisfies that task's own Scan NFC step — two equivalent ways to finish a bound task, one scan either way**, and since both `"anytime"` and `"session"` modes now open the exact same standalone `TaskFormScreen`/`TimerScreen`, the mechanism is identical for both:
