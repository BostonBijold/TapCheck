...
**FAB "scan to open" shortcut** (`components/BottomNav.tsx`): when nothing is currently running, the FAB shows an NFC icon instead of resuming a timer. Tapping it calls `scanNfcTag()` directly (no task screen open yet), resolves the read UID to a task via `GET /api/tasks/by-nfc-uid?uid=<uid>` (open to any signed-in company user, not manager-gated — same "any employee on shift" philosophy as triggering an already-linked tap-to-trigger tag), then navigates to `/tasks?openTaskId=<id>&date=<localDate>`. `TasksView.tsx`'s "Handle URL params passed from FAB navigation" effect (already home to `autoStartNext`/`autoAddTask`/`autoResumeTimer`) picks up `autoOpenTaskId` there and opens that task exactly like tapping "Start task" on it directly — `setTimerItem`, no separate code path. **This only opens the task, it does not complete it** — the task still goes through its normal fill-in flow, and if it has a bound tag, still needs its own separate Scan NFC step to finish. A tag that doesn't resolve to any task (unbound, wrong company, or scan failed/cancelled) shows a transient message in the same pill slot the active-timer resume pill uses, not an error page — the FAB stays usable either way.
